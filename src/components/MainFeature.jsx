          ...
          {stages.map(stage => (
            ...
            >
              ...
              <span className="ml-2 text-xs opacity-75">
                ({stage.id === 'all' ? candidates.length : candidates.filter(c => c.stage === stage.id).length})
              </span>
            </motion.button>
          ))}
        </div>
              ...
              <div className="flex items-center justify-between">
                      ...
                      {candidate.assessmentScores?.length > 0 && (
                        <>
                          ...
                          <span className="text-accent font-medium">
                            Score: {Math.round(candidate.assessmentScores.reduce((a, b) => a + b, 0) / candidate.assessmentScores.length)}%
                          </span>
                        </>
                      )}
                    </div>
                ...
                <div className="flex items-center space-x-3">
                  <div className={`px-3 py-1 rounded-full text-xs font-medium ${
                    candidate.stage === 'Applied' ? 'bg-gray-100 text-gray-700' :
                    candidate.stage === 'Screening' ? 'bg-blue-100 text-blue-700' :
